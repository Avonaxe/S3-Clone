---
name: s3-architect
description: Rules and architecture guidance for a lightweight S3 clone using Spring Boot, React, PostgreSQL and Docker.
---
...
The agent must focus exclusively on delivering these features:
    ...
    Identity: User registration, login, and JWT-based authentication.

    Bucket Management: Create bucket, delete bucket, list user's buckets.

    Object Management: Upload object to bucket, download object, delete object.

    Retrieval: List bucket contents, retrieve object metadata.
...
8. Docker & Deployment Rules
    ...
    Services Required:
        ...
        postgres: Standard official PostgreSQL image.

        backend: Built via Dockerfile in the /backend directory. Must volume-mount the /storage directory to persist files outside the container.

        frontend: Built via Dockerfile in the /frontend directory (using Nginx or simple Node server for serving static files).
